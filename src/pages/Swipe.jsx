...
async function fetchViewerAndQueue(userId) {
  ...
  // Full ranked list — no top-10 cap
  const ranked = rankCandidates(viewer, candidates ?? [])
  return { viewer, queue: ranked.map(r => r.profile) }
}
  ...
  if (queue.length === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-rose-50 to-orange-50 flex flex-col items-center justify-center px-6 text-center">
        ...
        <p className="text-gray-500 mb-8">Check back soon — new students join every day.</p>
        <button className="btn-secondary" onClick={() => navigate('/top-ten')}>
          ← Back to Top Ten
        </button>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-orange-50 flex flex-col">
      ...
      <div className="flex items-center justify-between px-6 pt-6 pb-2">
        ...
        <h1 className="text-xl font-bold text-rose-500">UniSwipe</h1>
